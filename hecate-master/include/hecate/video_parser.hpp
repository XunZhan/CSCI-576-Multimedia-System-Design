...
namespace hecate {
  ...
  public:
    ...
    void parse_photo(const string& in_photo, hecate::parser_params opt,
  ...
  private:
    ...
    int read_photo( const string& in_photo, int step_sz,
                    double max_duration, bool ignore_rest);
    
    /* Split video into subVideo*/
    void split_into_subVideo(int min_shot_len=30);
    
    /* Filter first end last few frames */
    void filter_heuristic(double fltr_begin_sec=0,
                          double fltr_end_sec=0);
    
    void filter_heuristic(int fltr_begin_nfrms=0,
                          int fltr_end_nfrms=0);
