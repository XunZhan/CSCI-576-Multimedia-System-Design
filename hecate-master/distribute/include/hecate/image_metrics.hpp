...
#define VALIDATE(x) ( std::isnan(x) ) ? 0 : x
  ...
  inline float calc_difference( int f, const Mat& org_img1, const Mat& org_img2, float thrsh)
  {
    Mat img1, img2, gray_img1, gray_img2, ans;
    float diffSum = 0;
    cvtColor( org_img1, gray_img1, CV_BGR2GRAY );
    cvtColor( org_img2, gray_img2, CV_BGR2GRAY );
    gray_img1.convertTo( img1, CV_32FC1 );
    gray_img2.convertTo( img2, CV_32FC1 );
    int nPixel = img1.rows * img1.cols;
    
    cv::absdiff(img1, img2, ans);
    for (size_t i = 0; i<img1.rows; i++)
    {
      for (size_t j = 0; j<img1.cols; j++)
      {
        //float a = cv::abs(img1.at<float>(i,j)  -  img2.at<float>(i,j));
        float a = ans.at<float>(i,j);
        diffSum += a;
        //printf("%f ", a);
      }
      //printf("\n");
