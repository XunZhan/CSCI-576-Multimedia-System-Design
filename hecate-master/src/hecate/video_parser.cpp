...
//  if (opt.debug) {
//    for (int rr = 0; rr < _X_feat.rows; rr++) {
//      for (int kk = 0; kk < _X_feat.cols; kk++)
//        printf("%f ", _X_feat.at<float>(rr, kk));
//      printf("\n");
//    }
//    printf("\n\n\n\n\n\n\n\n\n");
//  }
                                      ...
                                      vector<int>& v_thumb_idx, vector<int>& v_cluster_id)
{
  ...
  int ret = read_photo( in_photo, opt.step_sz, opt.max_duration,
                        opt.ignore_rest);
  if( ret<0 ) {
    fprintf( stderr, "VideoParser: Failed to open input photo directory: %s\n",
             in_photo.c_str());
    return ;
  }
  ...
  if( nfrm_valid==0 ) {
    fprintf( stderr, "VideoParser: No valid photos in input photo directory: %s\n",
             in_photo.c_str());
    return ;
  }
  ...
  //
  // obtain hightlight photos -- the most high quality frame per cluster
  for(int i=0; i<ncluster; i++) {
    int max_idx = -1;
    double max_quality = numeric_limits<double>::min();
    for(int j=0; j<km_lbl.rows; j++) {
      if( km_lbl.at<int>(j) == i ) {
        double quality = calcQuality(j);
    ...
    v_thumb_idx.push_back( v_idxmap[max_idx] );
    v_cluster_id.push_back(km_lbl.at<int>(max_idx));
  }
  release_memory();
...
//  ? _nfrm_total : round(max_duration*_video_fps);
  ...
  while( true )
  {
    ...
    if (_nfrm_total >= s.size())
    {
      break;
    }
    MyImage img;
    img.setWidth(_video_width);
    img.setHeight(_video_height);
    const char * cstr = s[_nfrm_total].c_str();
    img.setImagePath(cstr);
    img.ReadImage();
  
    Mat frm_org = Mat(_video_height,_video_width,CV_8UC3);
    frm_org.data = new unsigned char[_video_width * _video_height*3];
    for ( int kk=0; kk<(_video_width * _video_height*3);kk++ )
    {
...
/*------------------------------------------------------------------------
 Read input photo and store as RGB and GRAY formats.
 ------------------------------------------------------------------------*/
int VideoParser::read_photo( const string& in_photo, int step_sz,
                             double max_duration, bool ignore_rest)
/*-----------------------------------------------------------------------*/
{
  if( _debug )
  ...
  vector<string> s = read_directory(in_photo);
  ...
  while( true )
  {
    ...
    // for photo we don't resize them
    _v_frm_rgb.push_back( frm_org );
...
/*------------------------------------------------------------------------
 Split video into subVideo and delete first and last frame
 ------------------------------------------------------------------------*/
...
void VideoParser::filter_heuristic(int fltr_begin_nfrms, int fltr_end_nfrms)
/*-----------------------------------------------------------------------*/
{
  for(int i=0; i<fltr_begin_nfrms; i++)
...
double VideoParser::calcQuality(int idx)
{
  double v_brightness;
  double v_sharpness;
  double v_uniformity;
  
  v_brightness = hecate::calc_brightness( _v_frm_rgb[idx]  );
  v_sharpness  = hecate::calc_sharpness(  _v_frm_gray[idx] );
  v_uniformity = hecate::calc_uniformity( _v_frm_gray[idx] );
  
  return  v_brightness + v_sharpness - v_uniformity/10;
}
